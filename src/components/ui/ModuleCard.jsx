...
const ModuleCard = ({ title, description, emoji, variant = "blue", progress = 0, href = "/modules", className }) => {
  ...
  return (
    <div className={cn("bg-card rounded-3xl p-6 border-4 transition-all duration-200 hover:translate-y-[-4px]", borderColors[variant], bgColors[variant], shadowColors[variant], "hover:shadow-[0_12px_0_hsl(200_50%_80%)]", className)}>
      <div className="flex items-start gap-4">
        <div className="text-6xl float-animation relative">
          {emoji}
          {isComplete && (
