...
const ModuleCard = ({ title, description, icon: Icon, emoji, variant = "blue", progress = 0, route = "/modules", className, locked = false }) => {
  ...
  return (
    <div className={cn(
      "bg-card rounded-2xl p-8 border-4 transition-all duration-200",
      borderColors[variant],
      bgColors[variant],
      shadowColors[variant],
      !locked && "hover:translate-y-[-4px] hover:shadow-[0_12px_0_hsl(200_50%_80%)]",
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <Button
            variant={locked ? "outline" : isComplete ? "success" : "outline"}
            size="sm"
            className="gap-2"
            onClick={() => !locked && navigate(route)}
            disabled={locked}
          >
            {locked ? "Locked 🔒" : isComplete ? "Review" : progress > 0 ? "Continue" : "Start"}
