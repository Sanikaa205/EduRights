...
import questions from "./module1QuizQuestions";
...
const saveProgressToAPI = async (moduleKey, progress) => {
  try {
    ...
    // Dispatch custom event to notify Modules page
...
export default function Module1Quiz() {
  ...
  const handleNext = () => {
    ...
    if (current < questions.length - 1) {
      ...
      setShowResult(true);
      saveProgressToAPI("module-1-progress", 100);
      submitQuizScoreToAPI("module-1", score);
    }
  };
  ...
  const keyLearnings = [
    ...
    "Education is a fundamental right, not a favor or privilege.",
    "Children cannot be stopped from learning because of poverty, caste, or gender.",
    "The government must ensure schools are accessible to all children.",
  ];
